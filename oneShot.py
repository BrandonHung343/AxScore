...
#			print('Coordinates of contour #{}: {},{}'.format(counter,cx,cy))
	...
	#
	#	biggestArea = max(cntAreas)
	#	tempAreas = copy.copy(cntAreas)
	#	biggestBoy = tempAreas.index(biggestArea)
	#	cntAreas.remove(biggestArea)
	#
	#	bigArea = max(cntAreas)
	#	bigBoy = tempAreas.index(bigArea)
	# except Exception as e:
	#	print(e)
	#	pass
	try: 
		biggestBoy = centroids[0]
		bigBoy = centroids[1]
	except Exception as e:
		print(e)
		pass
	...
	if biggestBoy is not None and bigBoy is not None:
		...
		# print(tempAreas[biggestBoy], tempAreas[bigBoy])
		print(centroids)
		p1 = (int(biggestBoy[0]), int(biggestBoy[1]))
		p2 = (int(bigBoy[0]), int(bigBoy[1]))
		print("p1:", p1, "p2:", p2)
